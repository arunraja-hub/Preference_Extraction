...
class PySC2EnvReduced(py_environment.PyEnvironment):
  ...
  def _reset(self):
    self._state = 0
    self._episode_ended = False
    return ts.restart(np.array([self.timesteps[0].observation.score_cumulative["score"]], dtype=np.int32))
  ...
  def _step(self, action):
    ...
    if self.timesteps[0].last():
      ...
      return ts.termination(np.array([self.timesteps[0].observation.score_cumulative["score"]], dtype=np.int32), self.timesteps[0].observation.score_cumulative["score"])
