...
class ExtractorTestCase(unittest.TestCase):
    def test_cnn_from_obs(self):
        model = tf_extractor.cnn_from_obs(None, 16, 64, 5, 2, 64, 8, 3, .01, .1, .1)
    ...
    def test_cnn_from_obs_stride(self):
        model = tf_extractor.cnn_from_obs(None, 16, 32, 5, 1, 64, 8, 4, .01, .1, .1)
        self.assertEqual(model.layers[0].filters, 16)
        for i in range(4):
            self.assertGreater(model.layers[i+1].filters, model.layers[i].filters)
    ...
    def test_cnn_from_obs_0(self):
        model = tf_extractor.cnn_from_obs(None, 16, 32, 0, 1, 64, 8, 0, .01, .1, .1)
        self.assertEqual(model.layers[-1].units, 1)

    def test_cnn_from_obs_stride3(self):
        model = tf_extractor.cnn_from_obs(None, 16, 32, 5, 3, 64, 8, 4, .01, .1, .1)
        self.assertEqual(model.layers[0].filters, 16)
        for i in range(4):
            self.assertGreater(model.layers[i+1].filters, model.layers[i].filters)
        self.assertEqual(model.layers[4].filters, 32)
        ...
        self.assertEqual(model.layers[0].strides[0], 1)
        self.assertEqual(model.layers[1].strides[0], 1)
        self.assertEqual(model.layers[2].strides[0], 2)
        self.assertEqual(model.layers[3].strides[0], 1)
        self.assertEqual(model.layers[4].strides[0], 1)
